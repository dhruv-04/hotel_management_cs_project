...
def billing_process(): #main billing function 
    ...
    def generate_bill():  #function denerating bill
        try:
            mycursor.execute('select * from  reg_portal where GuestID={}'.format(int(gid.get())))
            reg_data = mycursor.fetchone()
            mycursor.execute('select sum(service_cost) from room_service group by Guest_ID having Guest_ID={}'.format(int(gid.get())))
            ...
            header_label = tk.Label(root1,
                                    text='Final Bill',
                                    font='Arial 45 bold underline',
                                    bg='#480BAB',
                                    fg='white').place(x=400, y=10)
            ...
            room_service_label = tk.Label(root1,
                                text='Room Service Cost         :',
                                font='calibri 20 bold',
                                bg='#480BAB',
            ...
            if reg_data[8] == 'SB':
                ...
                mycursor.execute('update room_aval set room_available = room_available + 1 where room_code="SB";')
                mycon.commit()
            elif reg_data[8] == 'DB':
                bill = float(reg_data[6]) * 7000.0
                mycursor.execute('update room_aval set room_available = room_available + 1 where room_code="DB";')
                mycon.commit()
            elif reg_data[8] == 'LB':
                bill = float(reg_data[6]) * 15000.0
                mycursor.execute('update room_aval set room_available = room_available + 1 where room_code="LB";')
                mycon.commit()

            if room_service_cost == None:
                room_service_val = tk.Label(root1,
                                        text='0.0',
                                        font='calibri 20 bold',
                                        bg='#480BAB',
            ...
            bill_val = tk.Label(root1,
                            ...
                            font='calibri 20 bold',
                            bg='#480BAB',
                            fg='white').place(x=450, y=500)
            
            ok_button = tk.Button(root1,
                                text='OK',
                                font='ARIAL 15 bold',
                                bg='#361869',
                                fg='white',
                                border=10,
                                command=lambda:ok()).place(x=450, y=550, width=130, height=50)

        
            mycursor.execute('insert into past_guests values({},"{}","{}","{}","{}")'.format(reg_data[0], reg_data[1], reg_data[2], reg_data[3], reg_data[4]))
            mycon.commit()
            mycursor.execute('delete from reg_portal where GuestID = {}'.format(int(gid.get())))
            mycon.commit()
            mycursor.execute('delete from guest_login where username = "{}"'.format('guest_'+str(gid.get())))
            mycon.commit()
            mycursor.execute('delete from room_service where Guest_ID = {}'.format(int(gid.get())))
            mycon.commit()
