...
def registration():  #defining main registration function
    ...
    def submit():  #allotment of room and creating of guest id and password
        root.destroy()
        mycursor.execute('select GuestID,room_num from reg_portal;')
        ...
        mycon.commit()
        mycursor.execute('insert into guest_login values("{}","{}",{})'.format(user_data,password,room_num))
        mycon.commit()

        root1 = tk.Tk()
        root1.geometry('1000x520')
        root1.title('Registration Successful!')
        ...
        ok_button = tk.Button(root1,
                            ...
                            fg='white',
                            border=10,
                            command=lambda:ok()).place(x=450, y=350, width=130, height=50)
